...
class LoremPage extends React.Component<Props, State> {
  ...
  render(): React.ReactNode {
    return <div className="LoremPage">
      <h1>Generate Lorem Ipsum Placeholder Text</h1>
      <div className="buttons">
        <button className="btn btn-outline-primary" onClick={this.generate} title="Generate">Generate</button>
        <input type="number" title="number of items to generate" value={this.state.num} onChange={this.numberChanged}></input>
        <label htmlFor="paragraphsOption">
          <input type="radio" name="generateType" id="paragraphsOption" value="paragraphs"
            onChange={this.radioChanged} checked={this.state.generateType === GenerateType.paragraphs} />Paragraphs
            </label>
        <label htmlFor="wordsOption">
          <input type="radio" name="generateType" id="wordsOption" value="words"
            onChange={this.radioChanged} checked={this.state.generateType === GenerateType.words} />Words
              </label>
      </div>
      ...
      <p>From <a href="https://en.wikipedia.org/wiki/Lorem_ipsum">Wikipedia</a>: In publishing and graphic design,
      Lorem ipsum is a placeholder text commonly used to demonstrate the visual form of a document or a typeface without
      relying on meaningful content. Lorem ipsum may be used before final copy is available, but it may also be used to temporarily
      replace copy in a process called greeking, which allows designers to consider form without the meaning of the text influencing the design.</p>
      <p>Lorem ipsum is typically a corrupted version of De finibus bonorum et malorum, a first-century BCE text by the Roman statesman and philosopher Cicero, with words altered, added, and removed to make it nonsensical, improper Latin.</p>
      ...
      <p>This page uses the npm package <a href="https://www.npmjs.com/package/lorem-ipsum">lorem-ipsum</a> behind the scene.</p>
    </div>;
  }
}
